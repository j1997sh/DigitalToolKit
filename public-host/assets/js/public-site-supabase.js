...
(async()=>{
  ...
  if(form)await window.CPPublicSubmit.prepare(form,compliance);
  for(const f of root.querySelectorAll('.pub-action-form'))await window.CPPublicSubmit.prepare(f,compliance);
  if(form){
    form.addEventListener('submit',async e=>{
      ...
      const {error}=await window.CPPublicSubmit.rpc('public_submit_survey_attributed',{
      ...
      if(!error)form.reset();window.CPPublicSubmit.reset(form);
    })
  }
  ...
  root.querySelectorAll('.pub-action-form').forEach(actionForm=>{
    actionForm.addEventListener('submit',async e=>{
      ...
      const geography=await resolvePostcode(fd.get('postcode'));
      const {error}=await window.CPPublicSubmit.rpc('public_capture_action_attributed',{
      ...
      actionForm.appendChild(msg);button.disabled=false;button.textContent=old;if(!error)actionForm.reset();window.CPPublicSubmit.reset(actionForm);
    })
  });
