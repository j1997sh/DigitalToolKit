...
export async function onRequestPost({request,env}){const origin=allowedOrigin(request,env);if(request.headers.get('Origin')&&!origin)return json({error:'Origin not allowed'},403,'');
 ...
 const user=await sb(env,'/auth/v1/user','GET',null,auth);
 ...
 if(!/^[0-9a-f-]{36}$/i.test(domainId))return json({error:'Invalid domain'},400,origin);
 const domains=await sb(env,'/rest/v1/rpc/local_domains_detail','POST',{},auth);
 const d=(domains.data||[]).find(x=>x.id===domainId);
 if(!d)return json({error:'Domain not found'},404,origin);
 const cf=await fetch(`https://api.cloudflare.com/client/v4/accounts/${env.CLOUDFLARE_ACCOUNT_ID}/pages/projects/${encodeURIComponent(env.CLOUDFLARE_PAGES_PROJECT)}/domains`,{
  method:'POST',headers:{'authorization':`Bearer ${env.CLOUDFLARE_API_TOKEN}`,'content-type':'application/json'},body:JSON.stringify({name:d.hostname})
 });
 const result=await cf.json();
 if(!cf.ok||result?.success===false)return json({error:'Cloudflare could not add the domain.',details:result?.errors||[]},400,origin);
 return json({domain:result.result},200,origin);
}