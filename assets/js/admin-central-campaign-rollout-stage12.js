...
async function init(){
 ...
 let data=null,supporters=[],performance=[],facebookCreatives=[],metaSummary=null,metaGeo=[];
 ...
 function render(){
  ...
  archiveCentralCampaign.disabled=r.status==='archived';
   const spend=(metaGeo||[]).reduce((a,x)=>a+Number(x.spend||0),0),paidSupporters=(metaGeo||[]).reduce((a,x)=>a+Number(x.supporters||0),0),drafts=(metaSummary?.drafts||[]),audiences=drafts.filter(x=>x.audience_id).length,adsets=drafts.filter(x=>x.adset_id).length,ads=(metaSummary?.assets||[]).filter(x=>x.ad_id).length;centralCampaignKpis.innerHTML=[['Websites',`${s.live||0}/${s.sites||0} live`],['Advertising',`${audiences}/${s.sites||0} audiences · ${adsets} ad sets · ${ads} ads`],['Response',`${s.supporters||0} supporters`],['Paid',spend?`£${spend.toFixed(2)} · ${paidSupporters?`£${(spend/paidSupporters).toFixed(2)} / supporter`:'no conversions'}`:'£0.00']].map(x=>`<div class="admin-kpi"><span>${x[0]}</span><strong>${x[1]}</strong></div>`).join('');
 ...
 async function load(){const metaReq=sb.functions.invoke('campaign-platform-meta',{body:{action:'summary',rollout_id:id}});const [r,sr,pr,ar,cr,mr,gr]=await Promise.all([sb.rpc('org_admin_central_campaign_rollout',{p_rollout:id}),sb.rpc('org_admin_central_campaign_supporters',{p_rollout:id}),sb.rpc('org_admin_central_campaign_rollout_performance',{p_rollout:id}),sb.rpc('org_admin_facebook_audiences',{p_org:orgId}),sb.rpc('org_admin_facebook_creatives',{p_rollout:id}),metaReq,sb.rpc('org_admin_meta_geography_results',{p_rollout:id})]);if(r.error||sr.error||pr.error||ar.error||cr.error||gr.error)return msg((r.error||sr.error||pr.error||ar.error||cr.error||gr.error).message,true);data=r.data;supporters=sr.data||[];performance=pr.data||[];facebookCreatives=cr.data||[];metaSummary=mr?.data||null;metaGeo=gr.data||[];facebookAudienceRefs=new Map();applyStoredFacebookAudiences(ar.data||[]);render();}
 ...
 duplicateCentralCampaignRollout.onclick=async()=>{const title=prompt('Name for the duplicated campaign:',`${data.rollout.title} copy`);if(!title)return;const r=await sb.rpc('org_admin_duplicate_central_campaign_with_meta',{p_rollout:id,p_new_title:title});if(r.error)return msg(r.error.message,true);location.href='admin-central-campaign-rollout.html?id='+encodeURIComponent(r.data)};
 ...
 if(window.metaAdvertising)metaAdvertising.href=`admin-meta-ads.html?id=${encodeURIComponent(id)}`;
