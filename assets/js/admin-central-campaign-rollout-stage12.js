...
async function init(){
 ...
 pauseCentralCampaign.onclick=async()=>{if(!confirm('Pause this central campaign and its live microsites?'))return;const r=await sb.rpc('org_admin_set_central_campaign_rollout_status',{p_rollout:id,p_status:'paused'});if(r.error)return msg(r.error.message,true);await load()};
 endCentralCampaign.onclick=async()=>{if(!confirm('End this campaign? Historic supporter data will be retained.'))return;const r=await sb.rpc('org_admin_set_central_campaign_rollout_status',{p_rollout:id,p_status:'ended'});if(r.error)return msg(r.error.message,true);await load()};
